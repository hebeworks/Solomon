/* global Ember, moment, _, $, hebeutils */
...
export default Ember.Service.extend({
  ...
  solomonConfig: Ember.computed({
    get() {
      ...
      // Lets you simulate another app (see config/envIronment)
      ...
      switch (hostname) {
        ...
        case 'bid.preview.mysolomon.co.uk':
          ...
          break;
        case 'aware.mysolomon.co.uk':
          solomonConfig.name = 'aware';
          solomonConfig.title = 'Aware Technologies';
          solomonConfig.storyConfig.storyHandle = 'both';
          break;
      }
  ...
  encodeQuery(query) {
    ...
    const base64 = hebeutils.Base64.encode(json);
    return base64;
  },
  ...
  init() {
    ...
    if (!Ember.isEmpty(this.get('solomonConfig'))) {
      const solomonInit = this.get('solomonConfig.initMethod');
      // const target = this;
  ...
  authToken: Ember.computed({
    get() {
      return this.container.lookup('simple-auth-session:main').get('content.secure.jwt');
    },
  }),
...
/////////////////////////////////////////////////////////////////
  ...
  loadNHSRegions() {
    const _this = this;
    const url = `${this.get('hebeNodeAPI')}/nhsrtt/regions`;
    this.getData(url, true).then((regions) => {
  ...
  loadNHSTreatments() {
    const _this = this;
    const url = `${this.get('hebeNodeAPI')}/nhsrtt/treatments`;
    this.getData(url, true).then((treatments) => {
  ...
  onNHSSelectedRegionChange: function(){
    const providers = this.get('canvasSettings.nhsFilter.selectedRegion.providers');
    const ccgs = this.get('canvasSettings.nhsFilter.selectedRegion.ccgs');
...
/////////////////////////////////////////////////////////////////
  ...
  loadYWZones() {
    const _this = this;
    const url = `${this.get('hebeNodeAPI')}/yw-zones?distinctfield=waterSupplySystem&sortfield=waterSupplyZone`;
    this.getData(url, true).then((data) => {
      const zones = [{ id: 'all', text: 'All - May be slow' }];
  ...
  loadSubZones: function () {
    const _this = this;
    const selectedZone = this.get('canvasSettings.ywFilter.selectedZone');
    if (!Ember.isEmpty(selectedZone)) {
      const query = this.encodeQuery({ waterSupplySystem: selectedZone.id });
      const url = `${this.get('hebeNodeAPI')}/yw-zones?distinctfield=pressureManagementZone&sortfield=waterSupplyZone&query=${query}`;
      this.getData(url, true).then((data) => {
        const zones = [{ id: 'all', text: 'All' }];
        if (!Ember.isEmpty(data)) {
          data.forEach((zone) => {
            zones.push({ text: zone, id: zone });
          });
  ...
  loadDMAs() {
    const _this = this;
    const url = `${this.get('hebeNodeAPI')}/yw-zones?distinctfield=zoneRef&sortfield=zoneRef`;
    this.getData(url, true).then((data) => {
      const zones = [{ id: 'all', text: 'All' }];
      if (!Ember.isEmpty(data)) {
        data.forEach((zone) => {
  ...
  onYWSelectedHistoryChange: function () {
    const selectedHistory = this.get('canvasSettings.ywFilter.selectedHistory');
...
/////////////////////////////////////////////////////////////////
  ...
  groupSortCount: function (arr, prop, count, desc) {
      var grouped = _.groupBy(arr, function (_this) {
          return _this[prop];
      });
      var items = [];
      ...
      items = _.sortBy(items, function (_this) { return _this.count; });
